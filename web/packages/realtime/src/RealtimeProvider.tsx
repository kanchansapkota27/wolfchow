...
export interface RealtimeProviderProps {
  /** Channel is `orders:{restaurantId}`; no connection is made when null. */
  restaurantId: string | null
  children: ReactNode
}

/**
 * Subscribes to the `orders:{restaurantId}` Supabase Realtime broadcast
 * channel and fans events out to `subscribe(event, handler)` listeners.
 * Unlike the JWT-authenticated apps (tablet, admin, superadmin), this
 * provider takes `restaurantId` as an explicit prop rather than deriving it
 * from an auth context, so it works in public/unauthenticated apps (widget,
 * tracking) too.
 */
export function RealtimeProvider({ restaurantId, children }: RealtimeProviderProps) {
