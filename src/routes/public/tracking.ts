...
export function registerPublicTrackingRoutes(app: Hono<HonoEnv>): void {
  // ── GET /public/track/:tracking_token ──────────────────────────────────────
  // Token-only lookup used by the standalone tracking app. The tracking token
  // is a UUID and globally unique, so no slug is needed for security.
  app.get('/public/track/:tracking_token', async (c) => {
    ...
    if (!order) {
      console.error('[tracking/token] order_not_found token=%s db_error=%s', trackingToken, orderError?.message)
      return c.json({ error: 'order_not_found' }, 404)
    }

    const or = order as Record<string, unknown>
    const restaurantId = or.restaurant_id as string
  ...
  // ── GET /public/:slug/orders/:tracking_token ───────────────────────────────
